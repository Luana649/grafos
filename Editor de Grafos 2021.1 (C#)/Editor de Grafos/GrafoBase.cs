...
namespace Editor_de_Grafos
{
    public class GrafoBase : Panel
    {
        ...
        public static List<VerticeOld> vertices;
        public static ArestaOld[,] matAdj; // matriz de adjacências
        public static VerticeOld vMarcado;
        ...
        public GrafoBase()
        {
            ...
            n = 0;
            vertices = new List<VerticeOld>();
            vMarcado = null;
            matAdj = new ArestaOld[500, 500];
            exibirPesos = false;
        ...
        public VerticeOld getVertice(int v)
        {
            int i;
            VerticeOld aux;
        ...
        public VerticeOld getVerticeMarcado()
        {
            return vMarcado;
        }

        public ArestaOld getAresta(int i, int j)
        {
            return matAdj[i, j];
        }
        ...
        public List<VerticeOld> getAdjacentes(int v)
        {
            List<VerticeOld> lista = new List<VerticeOld>();
            for (int i = 0; i < n; i++)
                if (matAdj[v, i] != null)
        ...
        public void setVerticeMarcado(VerticeOld v)
        {
            if (vMarcado != null)
                vMarcado.desmarcar();
            vMarcado = v;
        ...
        {
            matAdj[i, j] = matAdj[j, i] = new ArestaOld(peso, Color.Blue, this);
            Refresh();
        }
        ...
        {
            VerticeOld v = new VerticeOld(num, rotulo, x, y, this);
            vertices.Add(v);
            this.Controls.Add(v);
        }
            ...
            try
            {
                ...
                for (i = 1; i < n; i++)
                {
                    ...
                    for (j = 0; j < i; j++)
                    {
                        ...
                        if (int.Parse(p[j]) > 0)
                            matAdj[i, j] = matAdj[j, i] = new ArestaOld(int.Parse(p[j]), Color.Black, this);
                    }
                }
                s.Close();
                this.Refresh();
        ...
        public void clicouVertice(VerticeOld v)
        {
            if (v.getMarcado())
            {
                v.desmarcar();
            ...
            else
            {
                ...
                if (vMarcado != null)
                {
                    ...
                    ArestaOld a = new ArestaOld(peso, Color.Black, this); // cria a nova aresta
